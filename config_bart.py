class Config(dict):
    def version_config(self, version):
        batch = 64
        val_batch = 16
        hp = {1: {'n_epoch': 20, 'batch': batch, 'valid_batch': val_batch, 'n_layer':6},
              2: {'n_epoch': 20, 'batch': batch, 'valid_batch': val_batch, 'n_layer':6}
             }
        self['n_epoch'] = hp[version].get('n_epoch', 20)
        self['pre_n_epoch'] = hp[version].get('n_epoch', 20)

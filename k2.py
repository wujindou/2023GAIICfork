...
def train(k=0):
    if WANDB:
        wandb.init(
                project="2023GAIIC",
                name="bart_k{}".format(k),
        )

    train_data = BartDataset(conf['train_file']+"_{}.csv".format(k))
    valid_data = BartDataset(conf['valid_file']+"_{}.csv".format(k))
    ...
    logger = Logger(conf['model_dir']+"_{}".format(k)+'/log%d.txt'%version, 'a')
    logger.log(conf)
    writer = SummaryWriter(conf['model_dir']+"_{}".format(k))
    
    Path(conf['model_dir']+"_{}".format(k)).mkdir(exist_ok=True, parents=True)
    for epoch in range(start_epoch, conf['n_epoch']):
        ...
        if epoch%3==0 or epoch > 20:
            checkpoint.save(conf['model_dir']+"_{}".format(k)+'/model_%d.pt'%epoch)
            model.eval()
            ...
            checkpoint.update(conf['model_dir']+"_{}".format(k)+'/model.pt', metrics = metrics.value())
            model.train()

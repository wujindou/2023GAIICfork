...
class BartModel2(nn.Module):
    ...
    def forward(self, inputs, attn_mask, outputs=None, infer=False):
        if outputs is None:
            if infer:
                pred = self.model.generate(inputs=inputs, max_length=80, min_length=2, num_beams=self.beam_size, use_cache=True, early_stopping=True, \
                ...
                return self.model.generate(inputs=inputs, max_length=80, min_length=2, num_beams=self.beam_size, use_cache=True, early_stopping=True, \
                        no_repeat_ngram_size=3, bos_token_id=0, pad_token_id=1, eos_token_id=2)
